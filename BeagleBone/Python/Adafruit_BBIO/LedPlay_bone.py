#!/usr/bin/env python
'''
	Program:		LedPlay_bone.py
	Version:		0.1.5
	Date:			26-Nov-2013
	Purpose:		To define fun functions for controlling a set of 8 LEDs

	Dependencies:	class LED from the file LED_bone.py
...
class LedPlay (LED):
	...
	BitPins = [ "P9_11", "P9_12", "P9_13", "P9_14", "P9_15", "P9_16", "P9_21", "P9_22" ]

	BitStates = [ False, True, False, True, False, True, False, True ]

	BitValues = [ 1, 2, 4, 8, 16, 32, 64, 128 ]

	LedColors = [ "Red", "Red", "Red", "Red", "Red", "Red", "Red", "Red" ]
	...
	def oneKnightCycle (self, ms):
		...
		for ledNr in range(1, 7, 1):
			led = self.LEDs[ledNr]
			led.bit.toggle(ms)

		# Reverse it (bit 7 to bit 0)
		for ledNr in range(7, -1, -1):
			led = self.LEDs[ledNr]
			led.bit.toggle(ms)
...
def Demo ():
	...
	# Count in binary UP from 0 to 255, then up from 15 to 63 and back down
	play.Counting()
	...
	play.Pattern(nrCycles, cycleTime)
